...
Route::get('v1/pagos/grupos/{ciclo}/{page}', function (Request $request, $ciclo, $page) {
    if ($request->header('Authorization') == "cepreuna_v1_api") {
        $limit = 80;
        $offset = ($page - 1) * $limit;
        $results = DB::select("SELECT id, fecha as fecha, created_at, nro_documento as dni, voucher FROM pagos WHERE created_at>=? AND voucher NOT LIKE '%.pdf' order by fecha ", [$ciclo]);
        $total_count = count($results);
        $totalPages = ceil($total_count / $limit);
        $results = array_slice($results, $offset, $limit);

        $results_pdf = [];
        $results_pdf = DB::select("SELECT id, fecha as fecha, created_at, nro_documento as dni, voucher FROM pagos WHERE created_at>=? AND voucher LIKE '%.pdf' order by fecha", [$ciclo]);
